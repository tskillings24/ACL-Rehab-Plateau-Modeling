...
df = pd.read_csv('ACL Master Longitudinal_clean.csv')
...
df = df.merge(plateau_df[['Athlete ID', 'Plateau']], on='Athlete ID', how='left')
analysis_df = df[df['Days_Post_Op'] <= 60].copy()
...
df.to_csv('ACL Master Longitudinal_clean2.csv')
