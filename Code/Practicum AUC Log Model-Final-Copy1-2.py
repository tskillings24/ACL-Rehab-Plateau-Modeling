...
df = pd.read_csv('ACL Master Longitudinal_clean2.csv')
...
thresholdLSI = -model_lsi.intercept_[0] / model_lsi.coef_[0][0]
thresholdLSI
...
ml_df_torque = analysis_df.dropna(subset=['Quad_Torque_Surgical'])

X_torque = ml_df_torque[['Quad_Torque_Surgical']]
y_torque = ml_df_torque['Plateau'].astype(int)

model_torque = LogisticRegression(random_state=42).fit(X_torque, y_torque)

torque_auc = roc_auc_score(y_torque, model_torque.predict_proba(X_torque)[:,1])


# In[20]:


# Calculate Torque Threshold
...
# Data for multivariate model (must have both variables)
ml_df_multi = analysis_df.dropna(subset=['Quad_LSI_Surgical', 'Quad_Torque_Surgical'])

X_multi = ml_df_multi[['Quad_LSI_Surgical', 'Quad_Torque_Surgical']]
y_multi = ml_df_multi['Plateau'].astype(int)
X_multi = ml_df_multi[['Quad_LSI_Surgical', 'Quad_Torque_Surgical']]
model_multi = LogisticRegression(random_state=42).fit(X_multi, y_multi)
multi_auc = roc_auc_score(y_multi, model_multi.predict_proba(X_multi)[:,1])
...
# *NOTE THESE VALUES DO NOT MATCH THE REAL DATA SET*
# 
